...
SCENARIO = 'H2EV10' #Add scenario to read file name
...
if filetype_input == 'gdx':
    if COMMODITY == 'Electricity':
        var_list = []
        var_list = var_list + ['EL_DEMAND_YCR']
    if COMMODITY == 'H2':
        var_list = [] 
        var_list = var_list + ['H2_DEMAND_YCR']
    if COMMODITY == 'Heat':
        var_list = [] 
        var_list = var_list + ['H_DEMAND_YCRA']
...
if filetype_input == 'gdx':
    ...
    #Recover data
        if COMMODITY == 'Electricity':
            df_demand = all_df['EL_DEMAND_YCR']

        if COMMODITY == 'H2':
            df_demand =  all_df['H2_DEMAND_YCR']
        
        if COMMODITY == 'Heat':
            df_demand =  all_df['H_DEMAND_YCRA']
...
df_demand.loc[:, 'Y'] = df_demand['Y'].astype(int)
df_demand = df_demand[df_demand['Y'] == year]     

#df_demand = df_demand[df_demand['VARIABLE_CATEGORY'] == 'EXOGENOUS'] 
#sum values
df_demand = pd.DataFrame(df_demand.groupby(['RRR', sum_column])['Value'].sum().reset_index())

#sum values
df_demand = pd.DataFrame(df_demand.groupby(['RRR'])['Value'].sum().reset_index())
...
merged_df = geo_df.merge(df_demand, on='RRR')
...
total_demand = merged_df['Value'].sum()

cbar_label = f'{COMMODITY} Demand TWh ({year})'

if COMMODITY == 'H2':
    cbar_label = f'H\u2082 Demand TWh ({year})'
...
merged_df.plot(column='Value', cmap=net_colour, facecolor=[.9, .9,.9], edgecolor='grey',
linewidth=.2, ax=ax)
ax.axis('off')

# Add legend
sm = plt.cm.ScalarMappable(cmap=net_colour, 
                           norm=plt.Normalize(vmin=merged_df['Value'].min(), 
...
if COMMODITY == 'H2':
    ax.text(0.02, 0.92, f'Final H\u2082 demand: {total_demand:.0f} TWh', transform=ax.transAxes, fontsize=12)
else:
    ax.text(0.02, 0.92, f'Final {COMMODITY} demand: {total_demand:.0f} TWh', transform=ax.transAxes, fontsize=12)
...
if not os.path.isdir('output/Demand'):
        os.makedirs('output/Demand')
        
output_dir = 'output/Demand'
