...
SCENARIO = 'V12_NOIMPORT_SALTCAVERNS_CCS_HighELECTROLYSIS_NOBLUE' #Add scenario to read file name
...
show_flow_arrows = 'YES' #Yes or NO, net flow arrow
show_label = 'YES'   # Yes or No
...
if filetype_input == 'gdx':
    if COMMODITY == 'Electricity':
        ...
        if LINES == 'Capacity' or LINES == 'CongestionFlow' or LINES == 'Flow': 
            var_list = var_list + ['G_CAP_YCRAF', 'X_CAP_YCR', 'X_FLOW_YCR']
    ...
    if COMMODITY == 'H2':
        var_list = []
        if LINES == 'Capacity' or LINES == 'CongestionFlow' or LINES == 'Flow': 
            var_list = var_list + ['G_CAP_YCRAF', 'XH2_CAP_YCR','XH2_FLOW_YCR']
        if LINES == 'Flow' or LINES == 'CongestionFlow':
...
fig, ax = plt.subplots(figsize=(12, 12), subplot_kw={"projection": projection}, dpi=200)
...
for i,row in df_capacity.iterrows(): 
    ...
    #Plot the lines
    if not(np.isnan(cap)):
        #plt arrows for the direction of flow 
        ...
        if show_flow_arrows == 'YES':
            if df_capacity.loc[i,'Value'] >= label_min:
                if df_capacity.loc[i,'Max_flow'] == 'True':
                    ...
                    if cat == 'cluster':
                        head_length = 2*width[0]/10
                        head_width = 1.25*width[0]/10 
                    else:
                        head_length = 2*width/10
                        head_width = 1.25*width/10
                    #Choose arrow style
                    arrow = "-|>"
                    joinstyle = 'round'
